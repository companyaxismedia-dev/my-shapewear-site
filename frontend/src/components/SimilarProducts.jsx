"use client";

import { useEffect, useMemo, useState } from "react";
import { useRouter } from "next/navigation";

const API_BASE =
  typeof window !== "undefined" &&
  (window.location.hostname === "localhost" ||
    window.location.hostname === "127.0.0.1")
    ? "http://localhost:5000"
    : "https://my-shapewear-site.onrender.com";

export default function SimilarProductsAdvanced({
  currentProduct,
}) {
  const router = useRouter();
  const [allProducts, setAllProducts] = useState([]);
  const [loading, setLoading] = useState(true);

  /* ================= FETCH ALL PRODUCTS ================= */
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch(
          `${API_BASE}/api/products?limit=200`
        );
        const data = await res.json();
        if (data.success) {
          setAllProducts(data.products);
        }
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
      }
    };

    fetchProducts();
  }, []);

  /* ================= TOKENIZER ================= */
  const tokenize = (text = "") =>
    text
      .toLowerCase()
      .replace(/[^a-z0-9\s]/g, "")
      .split(" ")
      .filter((w) => w.length > 2);

  /* ================= ADVANCED MATCHING ================= */
  const similarProducts = useMemo(() => {
    if (!currentProduct || !allProducts.length)
      return [];

    const currentTokens = tokenize(currentProduct.name);

    const scored = allProducts
      .filter((p) => p._id !== currentProduct._id)
      .map((product) => {
        const productTokens = tokenize(product.name);

        const matchedWords = currentTokens.filter((w) =>
          productTokens.includes(w)
        );

        const wordScore = matchedWords.length * 3;

        const categoryBoost =
          product.category === currentProduct.category
            ? 5
            : 0;

        const intersection = matchedWords.length;
        const union = new Set([
          ...currentTokens,
          ...productTokens,
        ]).size;

        const jaccard =
          union > 0 ? (intersection / union) * 10 : 0;

        const finalScore =
          wordScore + categoryBoost + jaccard;

        return { ...product, finalScore };
      })
      .filter((p) => p.finalScore > 6)
      .sort((a, b) => b.finalScore - a.finalScore)
      .slice(0, );

    return scored;
  }, [currentProduct, allProducts]);

  if (loading || !similarProducts.length) return null;

  /* ================= UI ================= */
  return (
    <div className="max-w-[1400px] mx-auto px-6 pb-20">
      <h2 className="text-xl font-semibold mb-6 tracking-wide">
        SIMILAR PRODUCTS
      </h2>

      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
        {similarProducts.map((item) => (
          <div
            key={item._id}
            onClick={() =>
              router.push(`/product/${item.slug}`)
            }
            className="cursor-pointer group bg-white rounded-lg overflow-hidden hover:shadow-xl transition-all duration-300"
          >
            {/* Image */}
            <div className="overflow-hidden bg-gray-50">
              <img
                src={
                  item.variants?.[0]?.images?.[0]
                    ? API_BASE +
                      item.variants[0].images[0]
                    : "/placeholder.jpg"
                }
                alt={item.name}
                className="w-full aspect-[3/4] object-cover transition-transform duration-500 group-hover:scale-105"
              />
            </div>

            {/* Details */}
            <div className="p-3">
              <h3 className="text-sm font-medium truncate text-gray-800 group-hover:text-black transition">
                {item.name}
              </h3>

              <div className="mt-1 text-sm font-semibold text-gray-900">
                ₹{item.price}
              </div>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}
