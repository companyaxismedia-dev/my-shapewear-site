...
export default function TrackOrder() {
  ...
  const handleTrack = async (e) => {
    ...
    try {
      // Backend route jo phone number query se search karega
      const res = await fetch(`http://localhost:5000/api/orders/track?phone=${phone}`);
      const data = await res.json();

      if (data.success && data.order) {
        setOrderData(data.order);
      } else {
        setError("Order nahi mila! Kripya registered number check karein.");
      }
    } catch (err) {
      setError("Server connection fail ho gaya.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleTrack} className="relative mb-12 group">
          <div className="relative flex items-center bg-white rounded-2xl shadow-xl border-2 border-white overflow-hidden">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="bg-[#041f41] text-white px-8 py-5 font-black uppercase text-sm hover:bg-pink-600 transition-all flex items-center gap-2"
            >
              {loading ? "Finding..." : "Track Order"}
            </button>
          </div>
        </form>
              ...
              <div className="bg-white rounded-[2.5rem] p-8 shadow-2xl border border-gray-50">
                <div className="flex justify-between items-start mb-10">
                  <div>
                    ...
                    <h2 className="text-2xl font-black text-[#041f41] uppercase italic">{orderData.status}</h2>
                  ...
                  <div className="text-right">
                    ...
                    <p className="font-bold text-blue-600">#{orderData._id?.slice(-8).toUpperCase()}</p>
                  </div>
                </div>
                ...
                <div className="relative flex justify-between px-2">
                  ...
                  <div className="flex flex-col items-center z-10">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center border-4 border-white shadow-md ${orderData.trackingId ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-400'}`}>
                      <Truck size={20} />
                    </div>
                  ...
                  <div className="absolute top-5 left-0 w-full h-1 bg-gray-100 -z-0">
                    <div 
                      ...
                      style={{ width: orderData.status === "Delivered" ? "100%" : orderData.trackingId ? "50%" : "5%" }}
                    ></div>
                  </div>
                </div>
                ...
                {orderData.trackingId && (
                  <div className="mt-8 bg-blue-50 p-4 rounded-2xl flex items-center justify-between border border-blue-100">
                    <span className="text-xs font-bold text-blue-900 uppercase">AWB / Tracking ID: {orderData.trackingId}</span>
                  </div>
                )}
              </div>

              {/* Grid Layout for Products and Info */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Items from orderData.products */}
                <div className="bg-white p-6 rounded-[2rem] shadow-lg border border-gray-50">
                  ...
                  {orderData.products?.map((item, i) => (
                    <div key={i} className="flex gap-4 mb-4 last:mb-0 pb-4 last:pb-0 border-b last:border-0 border-gray-50">
                      <div className="w-16 h-20 bg-gray-50 rounded-xl border flex items-center justify-center text-[10px] text-gray-400 uppercase font-bold">Image</div>
                      <div>
