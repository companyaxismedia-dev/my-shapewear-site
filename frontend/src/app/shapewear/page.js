...
// Shapewear Products Data (Generating 42 items)
export const shapeProducts = Array.from({ length: 42 }, (_, i) => {
  const num = i + 1;
  const currentPrice = 699 + (i * 7);
  return {
    id: `shapewear-${num}`,
    name: `BootyBloom Curve Sculpting Shapewear ${num}`,
    price: currentPrice,
    oldPrice: currentPrice + 900,
    img: `/image/shapewear/shapewear-${num}.jpg`,
    ...
    <div className="min-h-screen bg-white font-sans overflow-x-hidden text-[#ed4e7e]">
      ...
      <div className="max-w-[1600px] mx-auto flex">
        <aside className="hidden lg:block w-64 p-6 border-r border-pink-50 sticky top-[120px] h-[calc(100vh-120px)] overflow-y-auto no-scrollbar">
          ...
          {["Body Zone", "Control Level", "Size", "Price Range", "Material"].map((f) => (
      ...
      <div className="p-3 flex flex-col flex-grow bg-white">
        ...
        <div className="flex items-start justify-between gap-1">
          <div className="flex flex-col">
            ...
            <p className="text-[8px] text-pink-400 font-medium italic leading-none">(inclusive of all taxes)</p>
          </div>
...
function ProductDetailsModal({ product, onClose }) {
  ...
  const handleCartAdd = () => {
    ...
    alert("Success! Shapewear added to cart.");
    onClose();
  };
      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl overflow-hidden shadow-2xl relative animate-in zoom-in duration-300 flex flex-col md:flex-row max-h-[90vh]">
        ...
        <div className="md:w-1/2 p-6 md:p-8 overflow-y-auto no-scrollbar space-y-6">
          <div className="space-y-2">
            <span className="text-[10px] font-bold uppercase tracking-widest text-[#ed4e7e] bg-pink-50 px-2 py-1 rounded">Seamless Sculpt</span>
          ...
          <div className="grid grid-cols-2 gap-4 pt-6 border-t border-gray-100">
            <div className="flex items-center gap-2 text-[10px] font-bold text-gray-500 uppercase"><Truck size={14} className="text-[#ed4e7e]" /> Express Shipping</div>
            <div className="flex items-center gap-2 text-[10px] font-bold text-gray-500 uppercase"><ShieldCheck size={14} className="text-[#ed4e7e]" /> Skin Friendly</div>
          </div>
        </div>
      </div>
    </div>
