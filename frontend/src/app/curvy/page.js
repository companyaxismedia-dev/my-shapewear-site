...
export const curvyProduct = Array.from({ length: 42 }, (_, i) => {
  const num = i + 1;
  const currentPrice = 599 + (i * 9);
  const oldPrice = currentPrice + 1000;

  return {
    id: `curvy-${num}`,
    name: `BOOTYBLOOM CURVY COMFORT ${num}`,
    price: currentPrice,
    oldPrice: oldPrice,
    img: `/image/curvy/curvy-${num}.jpg`,
    discount: `${Math.floor(Math.random() * 15) + 40}%`,
    rating: (Math.random() * (5 - 4.2) + 4.2).toFixed(1),
    reviews: 95 + (num * 2),
    offer: "3 FOR 1099",
    sizes: ["34C", "36D", "38B", "40C", "42D"],
...
export default function CurvyPage() {
  ...
  // Exactly like your Tummy Control logic: prevents hydration mismatch and generates 42 products
  useEffect(() => {
    curvyProduct;
    setProducts(curvyProduct);
  }, []);
  ...
  return (
    <div className="min-h-screen bg-white font-sans overflow-x-hidden text-[#ed4e7e]">
      ...
      <div className="bg-pink-50 py-6 text-center border-b border-pink-100">
        ...
        <p className="text-[11px] text-[#ed4e7e] mt-1 italic opacity-80">Support and style for every curve</p>
      </div>
      ...
      <div className="max-w-[1600px] mx-auto flex">
        <aside className="hidden lg:block w-64 p-6 border-r border-pink-50 sticky top-[120px] h-[calc(100vh-120px)] overflow-y-auto no-scrollbar">
          ...
          {["Size", "Color", "Discount", "Fabric", "Price Range"].map((f) => (
...
function ProductDetailsModal({ product, onClose }) {
  ...
  const handleCartAdd = () => {
    ...
    alert("Success! Curvy comfort added to bag.");
    onClose();
  };
      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl overflow-hidden shadow-2xl relative animate-in zoom-in duration-300 flex flex-col md:flex-row max-h-[90vh]">
        ...
        <div className="md:w-1/2 p-6 md:p-8 overflow-y-auto no-scrollbar space-y-6">
          <div className="space-y-2">
            <span className="text-[10px] font-bold uppercase tracking-widest text-[#ed4e7e] bg-pink-50 px-2 py-1 rounded">Curvy Special</span>
              ...
              <div className="flex gap-3">
                {product.colors.map(c => (
                  <button key={c} onClick={() => setColor(c)} className={`w-10 h-10 rounded-full border-2 transition-all ${color === c ? 'border-[#ed4e7e] p-0.5' : 'border-gray-200'}`}>
                    <div className="w-full h-full rounded-full shadow-inner" style={{ backgroundColor: c === 'Nude' ? '#e3bc9a' : (c === 'Black' ? '#000000' : '#ffc0cb') }}></div>
                  </button>
                ))}
              </div>
            </div>
